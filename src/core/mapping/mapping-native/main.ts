...
import { IMappingModule, MappingModuleOptions, DEFAULT_MAP_HEADERS_OPTIONS } from "../i-mapping-module";
...
export class MappingModule implements IMappingModule {
                ...
                try {
                    ...
                    for (let i = 0; i < rowCount; i++) {
                        ...
                        mappedRows[i] = {
                            ...
                            __originalValue: this.options.preserveOriginalValue ? JSON.stringify(mapped) : undefined,
                            value: mapped   
                        };
    ...
    handleRemap = async (layout: LayoutBase, row: any, signal?: AbortSignal) => {
        ...
        let rowKeys: string[] = Object.keys(row);
        ...
        if (this.options.allowRemapColumns) {
            ...
            let canBeMapped = rowKeys.length >= layout.headers.filter(e => e.required).length;
            if (canBeMapped && this.options.onRemapFn) {
                ...
                while (!mapValue && attemps < 3) {
                    ...
                    let result = await this.options.onRemapFn?.(rowKeys, layout.headers);
                    if(result){
                        mapValue = result;
                    } else {
    ...
    handleAbortSignal = (abortSignal?: AbortSignal, step: string = 'mapping', id: string = 'mapping-native') => {
