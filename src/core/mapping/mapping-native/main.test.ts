...
describe("MappingModule", () => {
  ...
  describe("generateMapFromHeaders", () => {
    ...
    it("should handle case-insensitive matching", () => {
      const headers = [createMockLayoutHeader({ key: "name", caseSensitive: false })];
      const row = { NAME: "John" };
  ...
  describe("handleRemap", () => {
    ...
    it("should throw error when validation fails and remap not allowed", async () => {
      ...
      await expect(mappingModule.handleRemap(layout, row)).rejects.toThrow(
        "Mapping Error: Headers are not valid"
      );
    });
    ...
    it("should throw error if onRemapFn fails after 3 attempts", async () => {
      ...
      await expect(customModule.handleRemap(layout, row, undefined)).rejects.toThrow(
        "Mapping Error: Cannot be mapped"
      );
    ...
    it("should throw error when columns cannot be remapped", async () => {
      ...
      await expect(customModule.handleRemap(layout, row)).rejects.toThrow(
        "Mapping Error: Cannot be mapped"
      );
    });
  });
  ...
  describe("handleStream", () => {
    ...
    it("should preserve original value when preserveOriginalValue is true", async () => {
      ...
      expect(capturedChunk.rows[0].__originalValue).toBe(
