...
export class ExporterNativeModule implements IExporterModule {
    ...
    exportStream = async (inputStream: ReadableStream<{rows: RowObject[]}>, exportFn: (row: RowObject) => any, signal?: AbortSignal): Promise<ReadableStream> => {
    ...
    exportToCsv = async (inputStream: ReadableStream<{rows: any}>, totalRowsCount: number, filename:string, diccLabels?: Record<string, string>, onProgress?: (progress: { bytesWritten: number; rowsProcessed: number; percentage: number }) => void, signal?: AbortSignal) => {
        
        
        const fileStream = streamsaver.createWriteStream(filename.endsWith('.csv') ? filename : filename + '.csv');
        ...
        try {
            ...
            while (true) {
                ...
                if(onProgress) {
                    onProgress({ bytesWritten, rowsProcessed: rowCount, percentage: Math.round((rowCount / totalRowsCount) * 100) });
                }
            }
