...
import { IImportFileModule, ImportFileModuleOptions, DEFAULT_IMPORT_FILE_MODULE_OPTIONS, StreamControls, StreamResult } from '../i-import-file-module';
    ...
    createDataStream(file: File, signal?: AbortSignal) {
        ...
        const stream = new ReadableStream({
            start: (controller) =>{
                ...
                this.logger.updateStatus({ order: 1, progress: 0, status: 'running', step: 'createDataStream' });
                ...
                Papa.parse(file, {
                    ...
                    error: (err) => {
                        this.logger.log(`Error reading file: ${err.message}`, 'error', 'createDataStream', this.id);
                        this.logger.updateStatus({ order: 1, progress: 0, status: 'error', step: 'createDataStream' });
...
const handleAbortSignal = (signal?: AbortSignal, step: string = 'importFile-papaparse', id: string = 'importFile-papaparse') => {
    if (signal?.aborted) {
